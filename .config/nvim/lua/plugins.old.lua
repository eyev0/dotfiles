...
return require("packer").startup(
  function(use)
    ...
    use { 'tpope/vim-obsession', cond = not_vscode }
    use { "folke/which-key.nvim", config = function() require("which-key").setup() end, cond = not_vscode }
    -- text editor enhancements
    use { 'kyazdani42/nvim-tree.lua', config = function () require'conf.nvim_tree' end, cond = not_vscode }
    use { 'hoob3rt/lualine.nvim', config = function() require'conf.lualine' end, cond = not_vscode }
    use { 'voldikss/vim-floaterm', config = function () require'conf.floaterm' end, cond = not_vscode }
    use { 'glepnir/indent-guides.nvim', config = function () require'conf.indentguides' end, cond = not_vscode }
    use { 'mbbill/undotree', cond = not_vscode }
    use { 'kevinhwang91/nvim-bqf', cond = not_vscode }
    use { 'windwp/nvim-autopairs', config = function () require'conf.autopairs' end, cond = not_vscode }
    use { 'folke/todo-comments.nvim', config = function() require'todo-comments'.setup() end, cond = not_vscode }
    use { 'brooth/far.vim', config = function () require'conf.far' end, cond = not_vscode }
    use { 'folke/zen-mode.nvim', config = function() require'zen-mode'.setup() end, cond = not_vscode }
    -- tmux
    ...
    -- treesitter - top shit
    use { 'nvim-treesitter/nvim-treesitter', run = ':TSUpdate', config = function() require'conf.treesitter' end, cond = not_vscode }
    ...
    use { 'romgrk/nvim-treesitter-context', disable = true, cond = not_vscode }
    ...
    -- colors
    use { 'folke/tokyonight.nvim', config = function () require'colorscheme' end, cond = not_vscode }
    use { 'norcalli/nvim-colorizer.lua', config = function() require'colorizer'.setup() end, disable = true, cond = not_vscode }
end)
